...
- Добавлен канонический контекст проекта для новых сессий Codex, а `AGENTS.md` теперь требует при старте читать актуальную сводку и Git-историю, после значимых задач самостоятельно обновлять историю/контекст и фиксировать изменения commit/push.
- На главном экране Mini App карточка «Мои участки» растянута на всю ширину, действия полива и осадков выстроены под ней в один ряд и получили разные приглушённые голубые оттенки; общая мобильная форма больше не выходит за границы карточки.
- Чат-бот переведён в режим справки Mini App: `bot_started`, любые сообщения, старые команды и callback-кнопки теперь показывают только инструкцию открытия и использования приложения без доступа к прежним сценариям в чате.
- Из Mini App удалены карточки статуса сервиса и последнего расчёта; время расчёта перенесено в шапку хозяйства, коэффициент водного стресса `1` показывается как «нет», а форма полива использует методы из основного контекста без отдельного падающего запроса.
