...
- Главный экран Mini App приведён к палитре и геометрии сайта KORNIX: показано московское время последнего применённого расчёта, удалены карточки готовности и отдельного списка методов, а форма полива теперь открывается независимо от доступности справочника методов.
...
- Логгер рекурсивно скрывает токены, секреты, Authorization, cookie, initData и hash; добавлены серверные и frontend-тесты безопасности и пользовательских переходов.

## 2026-07-20

- Запись поливов и осадков переведена на отдельные MAX-bot endpoints KORNIX с единым пятиминутным debounce-расчётом; подтверждение сообщает пользователю, что пакет принят и будет рассчитан после завершения окна изменений.
...
- Исправлено подтверждение inline-callback в MAX: `/answers` больше не получает пустое тело, поэтому нажатия кнопок завершаются без HTTP 400.
